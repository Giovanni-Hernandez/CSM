...
# Generator of One Time Password (OTP)
...
def register_user():
    ...
    Label(register_screen, text="",bg="black").pack()
    Button(register_screen, text="Get OTP", width=15, height=1, font=("Tahoma", 11), background="#E03C2E",foreground="white", activeforeground="white", activebackground="#1BB022", command = get_otp).pack()

def get_otp():
    Label(register_screen, text="",bg="black").pack()
    Label(register_screen, text="Please save your OTP", bg="#1BB022",foreground="white",font=("Tahoma", 12),height="1", width="20").pack()
    username_info = username.get()
...
def login_verify():
    ...
    if username1 in list_of_files:
        ...
        if password1 in verify:
            #login_sucess()
            var = ver.verificate_otp(username1)
            if var:
                 login_sucess()
            else:
